...
const profile = () => {
  ...
  const getUser = () => {
    axios
      .get(process.env.REACT_APP_BACKEND_URLACKEND_URL + `/api/v1/user/`, { withCredentials: true })
      .then((data) => {
        ...
        setEmail(myEmail);
        axios
          .post(process.env.REACT_APP_BACKEND_URLACKEND_URL + `/api/v1/person_park/visited`, {
            email: myEmail,
          })
          .then((data) => {
  ...
  const logout = () => {
    axios
      .get(process.env.REACT_APP_BACKEND_URLACKEND_URL + `/api/v1/user/logout`, {
