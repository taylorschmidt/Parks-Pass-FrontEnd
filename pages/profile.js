...
const profile = () => {
    ...
    const getUser = () => {
        ...
        .then((data) => {
          ...
          setUser(userId)
          axios.post("http://localhost:8000" + `/api/v1/person_park/visited`, {email: myEmail})
          .then((data)=>{
    ...
    const logout = () => {
        axios.post("http://localhost:8000" + `/api/v1/user/logout`, { withCredentials: true }).then((data)=>{console.log(data)}).catch(err=>{console.log(err)})
